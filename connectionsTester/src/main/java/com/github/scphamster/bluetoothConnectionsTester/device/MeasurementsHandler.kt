...
class MeasurementsHandler(
    ...
    inner class MeasurementsToFileSaver {
        ...
        private fun setCellStyle(cell: Cell, workbook: XSSFWorkbook, status: PinConnectionsStatus) {
            ...
            when (status) {
                PinConnectionsStatus.AlteredConnectionsList -> {
                    style.setFillForegroundColor(xssfColorFromInt(context.getColor(R.color.pin_with_altered_connections)))
                }

                PinConnectionsStatus.DoubleChecked -> {
                    style.setFillForegroundColor(xssfColorFromInt(context.getColor(R.color.pin_with_double_checked_connections)))
                }
                ...
                PinConnectionsStatus.SubstantialDifferenceFound -> {
                    style.setFillForegroundColor(xssfColorFromInt(context.getColor(R.color.pincheck_substantial_difference_found)))
                }
            }
            ...
            val to_wrap = PreferenceManager.getDefaultSharedPreferences(context)
        ...
        private fun convertPinConnectionDifferencesToRichText(
            pin: Pin,
            max_resistance: Float
        ): Pair<XSSFRichTextString, Boolean> {
        ...
        suspend fun storeMeasurements(maximumResistance: Float): Boolean {
            ...
            for (congregation in pins_congregations) {
                ...
                for ((pin_counter, pin) in congregation.pins.withIndex()) {
                    ...
                    if (!pin.isHealthy) setCellStyle(
                        cell_for_this_pin_connections,
                        workbook,
                        PinConnectionsStatus.Unhealthy
                    )
                    else if (pin.connectionsListChangedFromPreviousCheck) setCellStyle(
                        cell_for_this_pin_connections,
                        workbook,
                        PinConnectionsStatus.AlteredConnectionsList
                    )
        ...
        suspend fun storeExpectedToMeasuredDifferences(maximumResistance: Float) {
            ...
            for (congregation in pins_congregations) {
                ...
                for ((pin_counter, pin) in congregation.pins.withIndex()) {
                    ...
                    val (cell_text, substantial_difference_found) = convertPinConnectionDifferencesToRichText(
                        pin,
                        maximumResistance
                    )
                    ...
                    if (substantial_difference_found) setCellStyle(
                        cell_for_this_pin_differences,
                        workbook,
                        PinConnectionsStatus.SubstantialDifferenceFound
                    )
                    else setCellStyle(cell_for_this_pin_differences, workbook, PinConnectionsStatus.DoubleChecked)
                }
